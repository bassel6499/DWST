...
export function applyCombatResult(attacker:UnitState,defender:UnitState,result:{
  ...
  const aStatus=attacker.personnel-attackerLosses<=0?'destroyed':(aDisorganization>=0.35||attacker.status==='disorganized'?'disorganized':attacker.status);
  const dStatus=defender.personnel-defenderLosses<=0?'destroyed':(dDisorganization>=0.35||defender.status==='disorganized'?'disorganized':defender.status);
