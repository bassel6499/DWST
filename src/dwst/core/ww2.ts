...
export interface WW2CombatInput { attacker: UnitState; defender: UnitState; terrainDefense: number; weather: number; surprise: number; artillerySupport: number; }
export interface WW2CombatResult { attackerLosses: number; defenderLosses: number; attackerEquipmentLosses: number; defenderEquipmentLosses: number; attackerEffectiveness: number; defenderEffectiveness: number; }
...
export function runWW2Turn(state: ScenarioState): SimulationReport {
  ...
  const units = Object.values(state.units).map(unit => ({ ...unit, history: [...unit.history] }));
