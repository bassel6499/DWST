...
export function validateAssignments(assignments:EquipmentAssignment[],equipment:EquipmentLedger[],links:EquipmentCrewLink[],specialists:SpecialistPool[]):string[] {
 ...
 for(const a of assignments){
  ...
  for(const id of a.personnelIds)if(!pool||!poolPersonnelIds.includes(id))errors.push(`Personnel ${id} is not in required crew pool for ${a.equipmentId}`);
 }
 return errors;
}
