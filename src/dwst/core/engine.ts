...
function resolveMovement(unit:UnitState,hours:number,rules:EngineCoefficients):MovementResolution{
 ...
 const distanceFactor=Math.min(1,hours/rules.movementHours),readiness=effectiveReadiness(unit,rules);
 ...
 const ratio=Math.min(1,completion); unit.position=interpolateGeographicPosition(unit.position,unit.order.destination,ratio);
 unit.fatigue=clamp(unit.fatigue+rules.movementFatigue*distanceFactor); unit.wear=clamp(unit.wear+rules.movementWear*distanceFactor);
 const fuelConsumed=Math.min(unit.fuel,rules.movementFuel*distanceFactor); unit.fuel-=fuelConsumed;
 return {fuelDelta:-fuelConsumed,event:{turn:0,phase:'movement',message:`${unit.name} executed its movement order at ${(completion*100).toFixed(0)}% expected efficiency.`,unitIds:[unit.id]}};
}
...
export function resolveTurn(state:ScenarioState,rules:EraRuleset=getEraRuleset(state.era),baseline?:SimulationBaseline,combatContexts?:Readonly<Record<string,CombatContext>>):SimulationReport{
 if(!rules) throw new Error('No ruleset selected');
 const events:SimulationEvent[]=[],deltas=new Map<string,ResourceDelta>();
 const recordDelta=(delta:ResourceDelta)=>deltas.set(delta.unitId,mergeResourceDelta(deltas.get(delta.unitId)??emptyResourceDelta(delta.unitId),delta));
 const hours=state.turnHours,turn=Math.floor(state.elapsedHours/Math.max(hours,1))+1;
 const units=Object.values(state.units).sort(byUnitId).map((unit)=>{
  ...
  if(movement.event){const event={...movement.event,turn};events.push(event);for(const entry of unitEventsFromSimulationEvent(event))next.history.push(entry.event);}
  recordDelta({unitId:next.id,personnel:0,equipment:0,ammunition:0,fuel:movement.fuelDelta});
  const scale=hours/rules.engine.movementHours; next.fatigue=clamp(next.fatigue+rules.engine.turnFatigue*scale); if(rules.logisticsEnabled)next.logistics=clamp(next.logistics-rules.engine.logisticsDrain*scale); next.readiness=clamp(next.readiness-rules.engine.readinessDrain*scale); next.combatPower=effectiveCombatPower(next,rules.engine);
 ...
 const engagements=rules.resolveCombat?resolveEngagements(resolvedState,combatContexts):[];
 for(const engagement of engagements){
  const attacker=resolvedState.units[engagement.attackerId],defender=resolvedState.units[engagement.defenderId]; if(!attacker||!defender)continue;
  const applied=applyCombatResult(attacker,defender,engagement); resolvedState.units[attacker.id]=applied.attacker; resolvedState.units[defender.id]=applied.defender;
  for(const delta of applied.resourceDeltas)recordDelta(delta);
  const event:SimulationEvent={turn,phase:'combat',message:engagement.result,unitIds:[attacker.id,defender.id]}; events.push(event);
  const lossesByUnit={[attacker.id]:{personnelLosses:-applied.resourceDeltas[0].personnel,equipmentLosses:-applied.resourceDeltas[0].equipment},[defender.id]:{personnelLosses:-applied.resourceDeltas[1].personnel,equipmentLosses:-applied.resourceDeltas[1].equipment}};
  for(const entry of unitEventsFromSimulationEvent(event,lossesByUnit))resolvedState.units[entry.unitId].history.push(entry.event);
 }
