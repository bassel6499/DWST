...
export function effectiveReadiness(
  unit: UnitState,
  coefficients: EngineCoefficients = DEFAULT_ENGINE,
): number {
  return clamp(
    unit.readiness *
...
export function effectiveCombatPower(
  ...
  return Math.max(
    0,
    unit.combatPower *
      effectiveReadiness(unit, coefficients) *
...
export function applyOrder(unit: UnitState, order: Order): UnitState {
  const reserveState = order.type === 'reserve' ? 'reserve' :
    order.type === 'attack' && unit.reserveState === 'reserve' ? 'alert' : unit.reserveState;
  return { ...unit, order, reserveState };
}
...
function resolveMovement(unit: UnitState, hours: number, rules: EngineCoefficients): MovementResolution {
  if (!unit.order?.destination || unit.status === 'destroyed' || unit.status === 'withdrawn') {
    return { fuelDelta: 0 };
  }
  ...
  const defaultMobility =
    0.45 + 0.25 * readiness + 0.20 * unit.commandQuality + 0.10 * (unit.equipment > 0 ? 1 : 0);
  const mobility = clamp(unit.mobility ?? defaultMobility);
  const completion =
    distanceFactor *
    (1 - rules.movementReadinessWeight + rules.movementReadinessWeight * readiness) *
    (1 - rules.movementCommandWeight + rules.movementCommandWeight * unit.commandQuality) *
    (0.75 + 0.25 * mobility);
  const distance = geographicDistanceMeters(unit.position, unit.order.destination);
  if (distance < 1e-6) return { fuelDelta: 0 };
  unit.position = interpolateGeographicPosition(unit.position, unit.order.destination, Math.min(1, completion));
  ...
  const fuelConsumed = Math.min(unit.fuel, rules.movementFuel * distanceFactor * (0.85 + 0.15 * mobility));
...
export function resolveTurn(
  ...
  combatContextProvider?: CombatContextProvider,
): SimulationReport {
  ...
  const units = Object.values(state.units).sort(byUnitId).map((unit) => {
    ...
    if (next.order?.type === 'reserve') next.reserveState = 'reserve';
    if (next.order?.type === 'attack' && next.reserveState === 'reserve') next.reserveState = 'alert';
    const movement = resolveMovement(next, hours, rules.engine);
    if (movement.event) {
      const event = { ...movement.event, turn };
      events.push(event);
      for (const entry of unitEventsFromSimulationEvent(event)) next.history.push(entry.event);
    }
    ...
    const suppressionRecovery = clamp(0.35 + 0.20 * next.commandQuality + 0.10 * next.cohesion);
    const disorganizationRecovery = clamp(0.015 + 0.025 * next.commandQuality + 0.020 * next.cohesion);
    next.suppression = clamp((next.suppression ?? 0) * (1 - suppressionRecovery * scale));
    next.disorganization = clamp((next.disorganization ?? 0) - disorganizationRecovery * scale);
    if (next.disorganization < 0.30 && next.status === 'disorganized') next.status = 'operational';
  ...
  const engagements = rules.resolveCombat ? resolveEngagements(resolvedState, combatContextProvider) : [];
