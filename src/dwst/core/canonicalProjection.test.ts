...
const registry:PersonnelRegistry={personnel:[
 ...
 {id:'p3',unitId:'u2',status:'assigned',qualifications:['tank'],experience:{}},
]};
const assignments:InstanceCrewAssignment[]=[
 {instanceId:'a',slot:1,personnelId:'p1',specialty:'tank'},
 {instanceId:'a',slot:2,personnelId:'p2',specialty:'tank'},
 {instanceId:'c',slot:1,personnelId:'p3',specialty:'tank'},
];

describe('projectCanonicalUnit',()=>{
 it('projects only canonical records owned by the requested unit',()=>{
  expect(projectCanonicalUnit('u1',registry,instances,assignments,definitions)).toEqual({
   unitId:'u1',personnel:2,equipment:2,equipmentOperational:1,equipmentDamaged:1,equipmentDestroyed:0,equipmentMissing:0,
   crewRequired:2,crewReady:2,equipmentReady:1,
  });
 });
 it('does not count personnel or assignments from another unit',()=>{
  expect(projectCanonicalUnit('u2',registry,instances,assignments,definitions)).toMatchObject({unitId:'u2',personnel:1,equipment:1,equipmentOperational:1,crewReady:1,equipmentReady:0});
 });
