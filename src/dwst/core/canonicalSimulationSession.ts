...
export function startCanonicalSimulation(state:ScenarioState,canonical:CanonicalState,rules:EraRuleset=getEraRuleset(state.era)):CanonicalSimulationSession{
 const scenarioErrors=validateScenario(state); if(scenarioErrors.length>0)throw new Error(`Scenario ${state.id||'<unknown>'} is not runnable: ${scenarioErrors.join('; ')}`);
 if(!rules)throw new Error('No ruleset selected');
 const capabilityErrors:string[]=[]; if(!rules.implemented)capabilityErrors.push('ruleset is not implemented'); if(!rules.resolveCombat)capabilityErrors.push('required combat implementation is missing');
 const validationErrors=validateEraRuleset(rules),errors=[...capabilityErrors,...validationErrors]; if(errors.length>0)throw new Error(`Era ${rules.id} is not runnable: ${errors.join('; ')}`);
 const projectedState=reconcileScenarioResourceAggregates(state,canonical);
 return {state:cloneScenario(projectedState),canonical:cloneCanonical(canonical),baseline:captureSimulationBaseline(projectedState),rules,provenance:createReplayProvenance(rules)};
}

function buildCombatContexts(state:ScenarioState,canonical:CanonicalState):Readonly<Record<string,CombatContext>>{
 return Object.fromEntries(Object.keys(state.units).map((unitId)=>{
  const projection=projectCanonicalUnit(unitId,canonical.personnel,canonical.equipment,canonical.crewAssignments,canonical.equipmentDefinitions,canonical.consumables);
  return [unitId,{attacker:projection,defender:projection}];
 }));
}
...
export function advanceCanonicalSimulation(session:CanonicalSimulationSession,policy:CombatAllocationPolicy):CanonicalSimulationSessionStepResult{
 ...
 const combatContexts=session.rules.resolveCombat?buildCombatContexts(working,session.canonical):undefined;
 const report=resolveTurn(working,session.rules,session.baseline,combatContexts);
 let canonical=cloneCanonical(session.canonical);
 for(const delta of report.resourceDeltas){
  if(delta.personnel>0||delta.equipment>0||delta.ammunition>0||delta.fuel>0)throw new Error(`Canonical resource reconciliation cannot commit resource increases for unit ${delta.unitId}`);
  const personnelLosses=-delta.personnel,equipmentLosses=-delta.equipment;
  if(personnelLosses>0||equipmentLosses>0){const commit=allocateCombatLosses(canonical,delta.unitId,{personnel:personnelLosses,equipment:equipmentLosses},policy);canonical=commitCombatResourceChanges(canonical,commit);}
  canonical={...canonical,consumables:commitCanonicalConsumableDelta(canonical.consumables,delta)};
 }
 ...
 return {session:{state:nextState,canonical,baseline:session.baseline,rules:session.rules,provenance},report};
}
