...
export type ReserveState = 'committed' | 'reserve' | 'alert' | 'moving' | 'counterattacking';
...
export interface UnitState {
  ...
  /** Generic spatial/condition inputs; era rules supply their interpretation. */
  frontageKm?:number; mobility?:number; reserveFraction?:number; reserveState?:ReserveState; suppression?:number; disorganization?:number;
...
export interface ScenarioState {
  id:string; name:string; era:EraId; scale:Scale; turnHours:number; elapsedHours:number; weather:number; terrain:number; intelLevel:number;
  /** Optional scenario terrain label. Era rules own the interpretation. */
  terrainType?:string;
  /** Optional generic visibility/LOS input. Era rules own the interpretation. */
  lineOfSight?:number;
  units:Record<string,UnitState>; events:SimulationEvent[]; locations?:ScenarioLocation[]; sensors?:Sensor[];
}
...
export interface ResourceDelta { unitId:string; personnel:number; equipment:number; ammunition:number; fuel:number; }
