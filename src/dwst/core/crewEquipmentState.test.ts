...
describe('projectInstanceEquipmentReadiness', () => {
 const definition:EquipmentDefinition={
  id:'modern-tank', name:'Modern Tank', era:'Modern', equipmentType:'tank',
  crewRequirementId:'Modern:tank:tankCrew',
 };
 const instance:EquipmentInstance={instanceId:'tank-1',definitionId:'modern-tank',unitId:null,status:'operational'};
 const registry:PersonnelRegistry={personnel:[
  {id:'p1',unitId:null,status:'assigned',qualifications:['tankCrew'],experience:{trained:1}},
  {id:'p2',unitId:null,status:'assigned',qualifications:['tankCrew'],experience:{trained:1}},
  {id:'p3',unitId:null,status:'assigned',qualifications:['tankCrew'],experience:{trained:1}},
  {id:'p4',unitId:null,status:'assigned',qualifications:['tankCrew'],experience:{trained:1}},
 ]};
 ...
 it('counts only personnel actually assigned to the requested instance',()=>{
  ...
  expect(result.crewReady).toBe(3); expect(result.crewShort).toBe(0); expect(result.usable).toBe(1);
 });
 ...
 it('does not make a damaged instance usable even with a complete crew',()=>{
  ...
  expect(result.crewReady).toBe(3); expect(result.usable).toBe(0);
 });
 ...
 it('does not count a qualified person assigned to another instance',()=>{
  ...
  const result=projectInstanceEquipmentReadiness(instance,assignments,registry,definition);
  expect(result.crewReady).toBe(2); expect(result.crewShort).toBe(1); expect(result.usable).toBe(0);
 });
});
