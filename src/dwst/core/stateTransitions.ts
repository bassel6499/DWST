...
function findEquipment(l:CanonicalResourceLedger,id:string){ return l.equipment.find(e=>e.designation===id||e.type===id); }

/** Applies validated resource operations as one transaction. On any error, returns the original ledger unchanged. */
export function applyResourceOperations(input:CanonicalResourceLedger, operations:ResourceOperation[]):TransitionResult {
  ...
  for(const op of operations){
    if(op.kind==='killPersonnel'){
      ...
      for(const s of next.specialists){ const i=s.personnelIds.indexOf(op.personnelId); if(i>=0){s.personnelIds.splice(i,1);s.personnel--;s.casualties++;if(s.qualified>0)s.qualified--; if(s.veteran>0)s.veteran--; else if(s.experienced>0)s.experienced--; else if(s.trained>0)s.trained--; }}
