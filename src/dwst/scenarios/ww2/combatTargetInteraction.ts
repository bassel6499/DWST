...
/** Category-level interaction; equipment definition capability is supplied by the era projection. */
export function calculateTargetInteraction(input: WW2CombatInput,attacker: WW2ForceCapability,defender: WW2ForceCapability): WW2TargetInteraction {
  const range = Math.max(0, input.distanceKm ?? 0);
  const directRange = Math.exp(-range / C.directFireRangeKm);
  ...
  const defenderArmor = clamp(defender.armor),attackerArmor = clamp(attacker.armor);
  const defenderInfantry = clamp(defender.infantry),attackerInfantry = clamp(attacker.infantry);
  return {
