...
export type WW2TerrainType =
  | 'open' | 'rolling' | 'forest' | 'urban' | 'ridge'
  | 'valley' | 'river' | 'marsh' | 'fortified';
...
export function calculateFrontage(unit: UnitState) {
  return Math.max(
    0.25,
    unit.frontageKm ?? C.defaultFrontageScaleKm * Math.sqrt(Math.max(unit.personnel, 1) / 1000),
  );
}
...
export function calculateTerrainExposure(input: WW2CombatInput) {
  ...
  const base = input.terrainType
    ? terrainExposure[input.terrainType]
    : clamp(1.25 - 0.35 * (terrain - 1), 0.55, 1.35);
  const weatherFactor = 1 - 0.20 * (1 - weather);
  return clamp(
    base * weatherFactor * clamp(input.lineOfSight ?? 1) * clamp(input.targetExposure ?? 1),
    0.25,
    1.35,
  );
}
...
export function calculateGeometry(
  ...
  const densityRatioA = clamp(
    Math.sqrt(densityA / Math.max(densityB, 1)), C.densityMin, C.densityMax,
  );
  const densityRatioB = clamp(
    Math.sqrt(densityB / Math.max(densityA, 1)), C.densityMin, C.densityMax,
  );
  const exposure = calculateTerrainExposure(input);
  const lineOfSight = clamp(
    input.lineOfSight ??
      (input.terrainType === 'forest' || input.terrainType === 'urban' ? 0.70 : 1),
  );
  return {
    distanceKm,
