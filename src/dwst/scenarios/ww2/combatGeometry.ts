...
import type { WW2CombatInput, WW2CombatPhase, WW2TerrainType } from './combatTypes';
...
export interface WW2GeometryFactors {
  readonly distanceKm: number; readonly phase: WW2CombatPhase; readonly rangeA: number; readonly rangeB: number;
  readonly frontA: number; readonly frontB: number; readonly engagedA: number; readonly engagedB: number;
  readonly densityA: number; readonly densityB: number; readonly densityRatioA: number; readonly densityRatioB: number;
  readonly lineOfSight: number; readonly exposureA: number; readonly exposureB: number; readonly terrainEffect: number;
}
const terrainExposure: Record<WW2TerrainType, number> = {
  open: 1.15, rolling: 1, forest: 0.68, urban: 0.55, ridge: 0.72,
  valley: 0.82, river: 0.60, marsh: 0.58, fortified: 0.48,
};
...
export function calculatePhase(distanceKm: number): WW2CombatPhase {
  if (distanceKm <= C.closeAssaultKm) return 'assault';
  if (distanceKm <= C.mainEngagementKm) return 'main_engagement';
  if (distanceKm <= C.phasePreparationRangeKm) return 'preparation';
  if (distanceKm <= C.phaseApproachRangeKm) return 'positioning';
  return 'approach';
}
...
export function calculateTerrainExposure(input: WW2CombatInput) {
  ...
  return clamp(base * (1 - 0.20 * (1 - weather)) * clamp(input.lineOfSight ?? 1) * clamp(input.targetExposure ?? 1), 0.25, 1.35);
}
export function calculateGeometry(input: WW2CombatInput, attacker: WW2ForceCapability, defender: WW2ForceCapability): WW2GeometryFactors {
  ...
  const frontA = calculateFrontage(input.attacker); const frontB = calculateFrontage(input.defender);
  const engagedA = clamp(frontB / frontA, C.minimumEngagedFraction, 1); const engagedB = clamp(frontA / frontB, C.minimumEngagedFraction, 1);
  const densityA = positive(input.attacker.personnel) / frontA; const densityB = positive(input.defender.personnel) / frontB;
  ...
  return { distanceKm, phase: calculatePhase(distanceKm), rangeA: calculateRangeFactor(distanceKm, attacker.artillery, attacker.air), rangeB: calculateRangeFactor(distanceKm, defender.artillery, defender.air), frontA, frontB, engagedA, engagedB, densityA, densityB, densityRatioA, densityRatioB, lineOfSight, exposureA: exposure, exposureB: exposure, terrainEffect: clamp(input.terrainDefense, 0.55, 1.55) };
}
