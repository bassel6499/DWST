...
function typeCapability(context:CombatUnitContext|undefined,types:string[]){
  if(!context)return 0; const total=Object.values(context.equipmentByType).reduce((a,b)=>a+b,0); const count=types.reduce((a,type)=>a+(context.equipmentByType[type]??0),0); return ratio(count,total);
}
/** Converts canonical equipment/crew records into WW2 combat capabilities. */
export function calculateForceCapability(context:CombatUnitContext|undefined):WW2ForceCapability{return{equipment:equipmentCapability(context),armor:typeCapability(context,['tank','assaultGun','tankDestroyer']),antiArmor:typeCapability(context,['antiTank','tankDestroyer']),artillery:typeCapability(context,['artillery','selfPropelledArtillery']),air:typeCapability(context,['aircraft','airSupport']),infantry:typeCapability(context,['infantry'])};}
/** Converts persistent unit condition into a combat-readiness vector. Zero ammunition now removes the ammunition component of firepower. */
export function calculateForceQuality(unit:UnitState):WW2ForceQuality{return{
  quality:(C.qualityTrainingWeight*clamp(unit.training)+C.qualityExperienceWeight*clamp(unit.experience)+C.qualityReadinessWeight*clamp(unit.readiness)+C.qualityMoraleWeight*clamp(unit.morale)+C.qualityCohesionWeight*clamp(unit.cohesion))*(1-0.55*clamp(unit.suppression))*(1-0.65*clamp(unit.disorganization)),
  ammunition:clamp(unit.ammunition), sustainment:C.sustainmentBase+C.sustainmentWeight*clamp(unit.logistics), wear:1-C.wearWeight*clamp(unit.wear), fatigue:1-C.fatigueWeight*clamp(unit.fatigue), suppression:clamp(unit.suppression), disorganization:clamp(unit.disorganization),
};}
