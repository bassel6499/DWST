...
export interface WW2EffectivenessFactors {
  readonly offenseA: number; readonly offenseB: number; readonly alpha: number; readonly beta: number;
  readonly attackerMobility: number; readonly defenderMobility: number; readonly command: number; readonly maneuver: number;
  readonly terrain: number; readonly weather: number; readonly surprise: number; readonly exposure: number;
  readonly attackerEquipment: number; readonly defenderEquipment: number;
}

export interface WW2AttritionResult {
  readonly attackerRemaining: number; readonly defenderRemaining: number;
...
export function calculateCommandAndManeuver(input: WW2CombatInput) {
  const aPosture = posture(input.attacker, false);
  const dPosture = posture(input.defender, true);
  return {
    command: clamp(input.command ?? (input.attacker.commandQuality - input.defender.commandQuality) * 0.35 + aPosture - dPosture, -0.5, 0.5),
    maneuver: clamp(input.maneuver ?? aPosture - dPosture, -0.5, 0.5),
  };
}
...
function phaseCommitment(phase: WW2CombatPhase, reserve: number) {
  const phaseFactor = phase === 'approach' ? C.approachCommitment : phase === 'positioning' ? C.approachCommitment : phase === 'preparation' ? C.preparationCommitment : phase === 'main_engagement' ? 0.80 : phase === 'assault' ? C.assaultCommitment : 0.95;
...
export function calculateEffectiveness(
  ...
  attackerCapability: WW2ForceCapability,
  defenderCapability: WW2ForceCapability,
  ...
  const supportA = {
    artillery: attackerCapability.artillery * 0.75,
    armor: attackerCapability.armor * 0.60,
    air: attackerCapability.air * 0.50,
    antiArmor: attackerCapability.antiArmor * 0.80,
  };
  const supportB = {
    artillery: defenderCapability.artillery * 0.75,
    armor: defenderCapability.armor * 0.60,
    air: defenderCapability.air * 0.50,
    antiArmor: defenderCapability.antiArmor * 0.80,
  };
  const beta = C.baseRate * offenseB * terrain * geometry.exposureB * (1 + supportB.artillery * target.artilleryTargetB) * (1 + supportB.air) * target.targetArmorB * target.armorTargetB * target.infantryTargetB * (1 - commandAndManeuver.maneuver * 0.45) * (1 - commandAndManeuver.command * 0.35) * (1 - surprise);
  const alpha = C.baseRate * offenseA * geometry.exposureA * (1 + supportA.artillery * target.artilleryTargetA) * (1 + supportA.air) * target.targetArmorA * target.armorTargetA * target.directFireA * (1 + commandAndManeuver.maneuver * 0.65) * (1 + commandAndManeuver.command * 0.45) * (1 + surprise);
  return {
    offenseA, offenseB, alpha, beta,
    attackerMobility: calculateMobility(input.attacker, attackerCapability),
    defenderMobility: calculateMobility(input.defender, defenderCapability),
    command: commandAndManeuver.command, maneuver: commandAndManeuver.maneuver,
    terrain, weather, surprise, exposure: (geometry.exposureA + geometry.exposureB) / 2,
    attackerEquipment, defenderEquipment,
  };
}

function calculateMobility(unit: UnitState, capability: WW2ForceCapability) {
  const derived = 0.35 + 0.20 * clamp(unit.readiness) + 0.15 * capability.armor + 0.10 * capability.equipment + 0.20 * clamp(unit.commandQuality);
  return clamp(unit.mobility ?? derived);
}

export function resolveAttrition(attackerPersonnel: number, defenderPersonnel: number, alpha: number, beta: number): WW2AttritionResult {
  const steps = 24;
  const dt = 1 / steps;
  ...
  for (let step = 0; step < steps && attackerRemaining > 0 && defenderRemaining > 0; step += 1) {
    ...
    const [k2a, k2b] = derivative(Math.max(0, attackerRemaining + k1a * dt / 2), Math.max(0, defenderRemaining + k1b * dt / 2));
    const [k3a, k3b] = derivative(Math.max(0, attackerRemaining + k2a * dt / 2), Math.max(0, defenderRemaining + k2b * dt / 2));
    const [k4a, k4b] = derivative(Math.max(0, attackerRemaining + k3a * dt), Math.max(0, defenderRemaining + k3b * dt));
...
export function calculateEffects(input: WW2CombatInput, capabilityA: WW2ForceCapability, capabilityB: WW2ForceCapability, effectiveness: WW2EffectivenessFactors, attrition: WW2AttritionResult): WW2CombatEffects {
  ...
  const fireA = clamp(0.20 + 0.45 * capabilityA.artillery + 0.25 * capabilityA.air + 0.10 * capabilityA.armor);
  const fireB = clamp(0.20 + 0.45 * capabilityB.artillery + 0.25 * capabilityB.air + 0.10 * capabilityB.armor);
  const intensityA = clamp(0.25 + 0.45 * lossRateA + 0.15 * capabilityA.artillery + 0.10 * capabilityA.air + 0.10 * Math.abs(effectiveness.maneuver));
  const intensityB = clamp(0.25 + 0.45 * lossRateB + 0.15 * capabilityB.artillery + 0.10 * capabilityB.air);
...
export function determineTacticalOutcome(input: WW2CombatInput, effectiveness: WW2EffectivenessFactors, effects: WW2CombatEffects, geometry: WW2GeometryFactors): WW2TacticalResult {
  ...
  const attackerCanExploit = effectiveness.attackerMobility > effectiveness.defenderMobility + C.outcomeExploitMobilityGap && effectiveness.maneuver > -0.05 && effectiveness.command > -0.20;
  ...
  if (input.defender.order?.type === 'withdraw' || effects.defenderDisorganizationDelta >= C.outcomeWithdrawalDisorganization) outcome = 'defender_withdraws';
  else if (localRatio < C.outcomeRepulsed) outcome = 'attacker_repulsed';
  else if (localRatio < C.outcomeStall) outcome = 'attacker_stalls';
  else if (attackerCanExploit && localRatio >= C.outcomeBreakthrough && effects.defenderDisorganizationDelta >= C.outcomeBreakthroughDisorganization) outcome = 'breakthrough';
  else if (attackerCanExploit && localRatio >= C.outcomePenetration && effects.defenderDisorganizationDelta >= C.outcomePenetrationDisorganization) outcome = 'penetration';
  const defenderReserveCommitted = (outcome === 'penetration' || outcome === 'breakthrough') && defenderReserve > 0.05;
  const attackerReserveCommitted = (outcome === 'penetration' || outcome === 'breakthrough') && attackerReserve > 0.05;
  ...
  const attackerAdvanceKm = outcome === 'breakthrough' ? C.movementBreakthroughKm : outcome === 'penetration' ? C.movementPenetrationKm : outcome === 'local_gain' ? C.movementLocalGainKm : 0;
  const defenderWithdrawalKm = outcome === 'defender_withdraws' ? C.movementWithdrawalKm : outcome === 'breakthrough' ? C.movementCounterattackKm : 0;
  return { outcome, localRatio, attackerAdvanceKm, defenderWithdrawalKm, defenderReserveCommitted, attackerReserveCommitted };
}
