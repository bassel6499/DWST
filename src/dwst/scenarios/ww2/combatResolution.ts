...
function reserveFraction(unit: UnitState) {
  return clamp(
    unit.reserveFraction ??
      (unit.order?.type === 'reserve' ? C.reserveOrderFraction : C.reserveDefaultFraction),
  );
}
...
export function calculateEffectiveness(
  ...
  const reactionDelayHours = Math.max(
    0,
    (1 - clamp(input.defender.commandQuality)) * 6 + reserveFraction(input.defender) * 2,
  );
  return {
    offenseA, offenseB, alpha, beta, attackerMobility, defenderMobility,
    command: commandAndManeuver.command, maneuver: commandAndManeuver.maneuver,
    terrain, weather, surprise,
    exposure: (geometry.exposureA + geometry.exposureB) / 2,
    attackerEquipment, defenderEquipment, reactionDelayHours,
  };
}
