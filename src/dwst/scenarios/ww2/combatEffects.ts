...
/** Converts the resolved fire into persistent casualties, condition and resource deltas. */
export function calculateEffects(
  ...
  const attackerEquipmentLosses = Math.min(
    ...
    Math.round(
      input.attacker.equipment * (C.equipmentLossBase + C.equipmentLossPersonnelWeight * lossRateA) *
      (C.equipmentLossCapabilityMin + (1 - C.equipmentLossCapabilityMin) * effectiveness.attackerEquipment) *
  ...
  const defenderEquipmentLosses = Math.min(
    ...
    Math.round(
      input.defender.equipment * (C.equipmentLossBase + C.equipmentLossPersonnelWeight * lossRateB) *
      (C.equipmentLossCapabilityMin + (1 - C.equipmentLossCapabilityMin) * effectiveness.defenderEquipment) *
