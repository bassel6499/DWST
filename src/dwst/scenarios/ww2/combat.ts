...
function frontage(unit: UnitState) {
  return Math.max(0.25, unit.frontageKm ?? WW2_COMBAT_COEFFICIENTS.defaultFrontageScaleKm * Math.sqrt(Math.max(unit.personnel, 1) / 1000));
}
...
function phaseFor(distanceKm: number): WW2CombatPhase {
...
function reserveCommitment(unit: UnitState) {
...
export function resolveWW2Combat(i: WW2CombatInput): WW2CombatResult {
  ...
  if (A0 <= 0 || B0 <= 0) {
    return {
      ...
      attackerEffectiveness: 0, defenderEffectiveness: 0, outcome: 'attacker_stalls', phase: 'approach', factors: {},
    };
  }
  ...
  const ammoA = 0.45 + 0.55 * clamp(i.attacker.ammunition), ammoB = 0.45 + 0.55 * clamp(i.defender.ammunition);
  const sustainA = 0.60 + 0.40 * clamp(i.attacker.logistics), sustainB = 0.60 + 0.40 * clamp(i.defender.logistics);
  const wearA = 1 - 0.40 * clamp(i.attacker.wear), wearB = 1 - 0.40 * clamp(i.defender.wear);
  const fatigueA = 1 - 0.45 * clamp(i.attacker.fatigue), fatigueB = 1 - 0.45 * clamp(i.defender.fatigue);
  const weather = 0.70 + 0.30 * clamp(i.weather), terrain = clamp(i.terrainDefense, 0.55, 1.55), surprise = clamp(i.surprise, -0.5, 0.5);
  const distanceKm = Math.max(0, i.distanceKm ?? 0), phase = phaseFor(distanceKm);

  const posture = i.attacker.order?.posture === 'aggressive' ? 0.15 : i.attacker.order?.posture === 'cautious' ? -0.10 : 0;
  const defenderPosture = i.defender.order?.posture === 'cautious' ? 0.12 : i.defender.order?.posture === 'aggressive' ? -0.08 : 0;
  const command = clamp(i.command ?? (i.attacker.commandQuality - i.defender.commandQuality) * 0.35 + posture - defenderPosture, -0.5, 0.5);
  const maneuver = clamp(i.maneuver ?? posture - defenderPosture, -0.5, 0.5);

  const attackerEquipment = equipmentCapability(i.attackerContext), defenderEquipment = equipmentCapability(i.defenderContext);
  ...
  const artilleryA = positive(i.artillerySupport ?? attackerArtillery * 0.75), artilleryB = positive(defenderArtillery * 0.75);
  const armorA = positive(i.armorSupport ?? attackerArmor * 0.60), armorB = positive(defenderArmor * 0.60);
  const antiArmorA = positive(i.antiArmor ?? attackerAntiArmor * 0.80), antiArmorB = positive(defenderAntiArmor * 0.80);
  const airA = positive(i.airSupport ?? attackerAir * 0.50), airB = positive(defenderAir * 0.50);

  const rangeA = rangeFactor(distanceKm, artilleryA, airA), rangeB = rangeFactor(distanceKm, artilleryB, airB);
  const targetArmorA = defenderArmor > 0 ? 1 + antiArmorA * (0.65 + 0.35 * defenderArmor) : 1 + 0.25 * antiArmorA;
  const targetArmorB = attackerArmor > 0 ? 1 + antiArmorB * (0.65 + 0.35 * attackerArmor) : 1 + 0.25 * antiArmorB;
  const armorTargetA = defenderArmor > 0 ? 1 + armorA * (0.35 + 0.65 * (1 - defenderAntiArmor)) : 1 + armorA * 0.75;
  const armorTargetB = attackerArmor > 0 ? 1 + armorB * (0.35 + 0.65 * (1 - attackerAntiArmor)) : 1 + armorB * 0.75;
  const artilleryTargetA = defenderArmor > 0 ? 0.75 + 0.25 * (1 - defenderArmor) : 1;
  const artilleryTargetB = attackerArmor > 0 ? 0.75 + 0.25 * (1 - attackerArmor) : 1;

  const frontA = frontage(i.attacker), frontB = frontage(i.defender);
  const engagedA = clamp(frontB / frontA, WW2_COMBAT_COEFFICIENTS.minimumEngagedFraction, 1);
  const engagedB = clamp(frontA / frontB, WW2_COMBAT_COEFFICIENTS.minimumEngagedFraction, 1);
  const densityA = A0 / frontA, densityB = B0 / frontB;
  ...
  const reserveA = reserveCommitment(i.attacker), reserveB = reserveCommitment(i.defender);
  ...
  const offenseA = (0.68 + 0.32 * aq) * ammoA * sustainA * wearA * fatigueA * attackerCapability * weather * rangeA * engagedA * densityRatioA * reserveA;
  const offenseB = (0.68 + 0.32 * dq) * ammoB * sustainB * wearB * fatigueB * defenderCapability * weather * rangeB * engagedB * densityRatioB * reserveB;

  const exposure = clamp(1.25 - 0.35 * (terrain - 1) - 0.20 * (1 - clamp(i.weather)), 0.55, 1.35);
  // beta = defender effectiveness against attacker; alpha = attacker effectiveness against defender.
  ...
  const la = Math.min(A0, Math.max(0, Math.round(A0 - A))), lb = Math.min(B0, Math.max(0, Math.round(B0 - B)));
  const lossRateA = ratio(la, A0), lossRateB = ratio(lb, B0);
  const equipmentLossA = Math.min(i.attacker.equipment, Math.round(i.attacker.equipment * (0.008 + 0.055 * lossRateA) * (0.65 + 0.35 * attackerEquipment) * (1 + armorA * 0.8)));
  const equipmentLossB = Math.min(i.defender.equipment, Math.round(i.defender.equipment * (0.008 + 0.055 * lossRateB) * (0.65 + 0.35 * defenderEquipment) * (1 + armorB * 0.8)));

  const fireA = clamp(0.20 + 0.45 * artilleryA + 0.25 * airA + 0.10 * armorA), fireB = clamp(0.20 + 0.45 * artilleryB + 0.25 * airB + 0.10 * armorB);
  ...
  const attackerSuppression = clamp(WW2_COMBAT_COEFFICIENTS.suppressionBase + WW2_COMBAT_COEFFICIENTS.suppressionFireWeight * fireB + WW2_COMBAT_COEFFICIENTS.suppressionLossWeight * lossRateB + 0.05 * Math.max(-surprise, 0));
  const defenderSuppression = clamp(WW2_COMBAT_COEFFICIENTS.suppressionBase + WW2_COMBAT_COEFFICIENTS.suppressionFireWeight * fireA + WW2_COMBAT_COEFFICIENTS.suppressionLossWeight * lossRateA + 0.05 * Math.max(surprise, 0));
  const attackerDisorganization = clamp(WW2_COMBAT_COEFFICIENTS.disorganizationBase + WW2_COMBAT_COEFFICIENTS.disorganizationSuppressionWeight * attackerSuppression + WW2_COMBAT_COEFFICIENTS.disorganizationLossWeight * lossRateA + 0.04 * Math.max(-command, 0));
  const defenderDisorganization = clamp(WW2_COMBAT_COEFFICIENTS.disorganizationBase + WW2_COMBAT_COEFFICIENTS.disorganizationSuppressionWeight * defenderSuppression + WW2_COMBAT_COEFFICIENTS.disorganizationLossWeight * lossRateB + 0.04 * Math.max(command, 0));
  ...
  const readinessA = -clamp(0.008 + 0.20 * lossRateA + 0.025 * intensityA + 0.04 * attackerSuppression, 0, 0.30);
  const readinessB = -clamp(0.008 + 0.20 * lossRateB + 0.025 * intensityB + 0.04 * defenderSuppression, 0, 0.30);
  ...
  const attackerMobility = mobility(i.attacker, attackerArmor, attackerEquipment), defenderMobility = mobility(i.defender, defenderArmor, defenderEquipment);
  const attackerScore = alpha * A0 * (1 - attackerSuppression) * (1 - attackerDisorganization) * (0.65 + 0.35 * attackerMobility);
  const defenderScore = beta * B0 * (1 - defenderSuppression) * (1 - defenderDisorganization) * (0.65 + 0.35 * defenderMobility) * terrain;
  const localRatio = attackerScore / Math.max(defenderScore, 1e-9);
  const attackerCanExploit = attackerMobility > defenderMobility + 0.08 && maneuver > -0.05 && command > -0.20;
  let outcome: WW2CombatOutcome;
  if (localRatio < 0.65) outcome = 'attacker_repulsed';
  else if (localRatio < 0.95) outcome = 'attacker_stalls';
  else if (localRatio < 1.35) outcome = 'local_gain';
  else if (attackerCanExploit && localRatio >= 1.75 && defenderDisorganization >= 0.15) outcome = 'breakthrough';
  else if (attackerCanExploit && localRatio >= 1.35 && defenderDisorganization >= 0.10) outcome = 'penetration';
  else if (defender.order?.type === 'withdraw' || defenderDisorganization >= 0.45) outcome = 'defender_withdraws';
  else outcome = 'local_gain';

  return {
    attackerLosses: la, defenderLosses: lb, attackerEquipmentLosses: equipmentLossA, defenderEquipmentLosses: equipmentLossB,
    attackerAmmunitionDelta: ammoDeltaA, defenderAmmunitionDelta: ammoDeltaB, attackerFuelDelta: fuelDeltaA, defenderFuelDelta: fuelDeltaB,
    attackerReadinessDelta: readinessA, defenderReadinessDelta: readinessB, attackerMoraleDelta: moraleA, defenderMoraleDelta: moraleB,
    attackerSuppressionDelta: attackerSuppression, defenderSuppressionDelta: defenderSuppression,
    attackerDisorganizationDelta: attackerDisorganization, defenderDisorganizationDelta: defenderDisorganization,
    attackerEffectiveness: 1 - Math.exp(-alpha * A0 * A0 / Math.max(B0, 1)),
    defenderEffectiveness: 1 - Math.exp(-beta * B0 * B0 / Math.max(A0, 1)), outcome, phase,
    factors: {
      attackerQuality: aq, defenderQuality: dq, ammoA, ammoB, sustainA, sustainB, wearA, wearB, fatigueA, fatigueB,
      weather, terrain, surprise, distanceKm, rangeA, rangeB, armorA, armorB, antiArmorA, antiArmorB,
      artilleryA, artilleryB, airA, airB, maneuver, command, attackerEquipment, defenderEquipment,
      frontA, frontB, engagedA, engagedB, densityA, densityB, densityRatioA, densityRatioB, reserveA, reserveB,
      attackerMobility, defenderMobility, attackerSuppression, defenderSuppression,
      attackerDisorganization, defenderDisorganization, localRatio, lossRateA, lossRateB,
    },
  };
}
