...
/**
 * WW2/industrial-era combat law. The square-law attrition equations remain
 * authoritative; the WW2 layer supplies historically meaningful modifiers,
 * equipment/crew readiness, combined-arms capabilities and explicit resource
 * consumption. The resolver is pure and never mutates its inputs.
 */
export function resolveWW2Combat(i:WW2CombatInput):WW2CombatResult{
  ...
  if(A0<=0||B0<=0){
    return {
      ...
      attackerEffectiveness:0,defenderEffectiveness:0,factors:{},
    };
  }
  ...
  const offenseA=(0.68+0.32*aq)*ammoA*sustainA*wearA*fatigueA*attackerCapability*weather;
  const offenseB=(0.68+0.32*dq)*ammoB*sustainB*wearB*fatigueB*defenderCapability*weather;

  const beta=0.00035*offenseA*(1+artilleryA)*(1+airA)*(1+armorA)*(1+antiArmorA*0.35)*(1+maneuver*0.65)*(1+command*0.45)*(1+surprise);
  const alpha=0.00035*offenseB*terrain*(1+artilleryB)*(1+airB)*(1+armorB)*(1+antiArmorB*0.35)*(1-maneuver*0.45)*(1-command*0.35)*(1-surprise);

  // Fixed substeps keep the square-law model stable when one side becomes
  // very small. Intermediate force values are never allowed below zero.
  ...
  for(let step=0;step<steps&&A>0&&B>0;step++){
  ...
  return {
    ...
    attackerEffectiveness:clamp(beta*B0*B0/Math.max(A0,1)),defenderEffectiveness:clamp(alpha*A0*A0/Math.max(B0,1)),
    factors:{attackerQuality:aq,defenderQuality:dq,ammoA,ammoB,sustainA,sustainB,wearA,wearB,fatigueA,fatigueB,weather,terrain,surprise,armorA,armorB,antiArmorA,antiArmorB,artilleryA,artilleryB,airA,airB,maneuver,command,attackerEquipment,defenderEquipment,lossRateA,lossRateB},
  };
}
