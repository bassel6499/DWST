...
| P2-S25 | Replay provenance and command history are incomplete | OPEN — NEXT | Persist an explicit ordered command journal plus ruleset/model version or content hash and any RNG state/seed required to reproduce a run. A state snapshot alone must be sufficient to identify the exact model used for replay. |
